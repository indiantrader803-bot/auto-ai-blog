...
/**
 * 📨 Resilient Resend Dispatcher
 * Automatically falls back to onboarding@resend.dev if the custom domain (thesmartmag.com)
 * is pending verification on Resend DNS.
 */
export async function dispatchResendEmail(params: {
  ...
  const primaryFrom = params.from || process.env.EMAIL_FROM || "The SmartMag <contact@thesmartmag.com>";
  ...
  try {
    ...
    // If domain is unverified (403), fallback to onboarding@resend.dev
    if (res.status === 403 && (errText.includes("not verified") || errText.includes("validation_error"))) {
      ...
      console.info(`[RESEND NOTICE] Custom domain pending verification. Retrying dispatch with ${fallbackFrom}`);
