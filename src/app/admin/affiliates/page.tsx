...
export default function AffiliateTrackingDashboardPage() {
  ...
  const handleAgenticSyncNow = async () => {
    ...
    try {
      ...
      if (json.success) {
        setData(json);
        setSyncSuccessMsg("✅ Agentic Affiliate Fetcher synced all platform conversions in real-time!");
  ...
  const avgCR = report?.aggregateConversionRate || `${((totalPurchases / Math.max(1, totalClicks)) * 100).toFixed(2)}%`;
  const avgEPC = report?.averageEpc || `$${(totalUSD / Math.max(1, totalClicks)).toFixed(2)}`;
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-10 max-w-7xl mx-auto space-y-6 sm:space-y-8 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 sm:pb-6 border-b border-slate-200 dark:border-slate-800">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
            Real-time affiliate URL tracking, purchaser conversion analytics, and daily automated agentic network sync.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          >
            ...
            <span>{isSyncing ? "Agent Syncing Networks..." : "Run Agentic Fetch Now"}</span>
          </button>
        </div>
      </div>
      ...
      {/* 2. Autonomous Agent Pulse Banner */}
      <div className="p-5 sm:p-6 rounded-3xl bg-gradient-to-r from-indigo-900/40 via-purple-900/20 to-slate-900/60 border border-indigo-500/20 shadow-sm relative overflow-hidden">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div className="flex items-start gap-3.5">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-emerald-400 font-bold uppercase">24/7 Autopilot</span>
              </div>
              <p className="text-xs text-slate-300 max-w-2xl leading-relaxed">
                {report?.topPerformingPlatform
                  ? `Active platform scan: ${report.topPerformingPlatform}. Conversion velocity nominal across all destination URLs.`
              ...
              <div className="flex items-center gap-3 text-[11px] text-slate-400 pt-1 font-mono">
                ...
                <span>•</span>
                <span>Monitored Platforms: {report?.totalPlatformsMonitored || 7}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Link
              ...
              className="px-3.5 py-2 rounded-xl bg-white/10 hover:bg-white/15 text-white text-xs font-bold border border-white/10 flex items-center gap-1.5 transition-colors"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="p-5 sm:p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-2">
          ...
          <div className="text-[11px] font-semibold text-amber-600 dark:text-amber-400">
            {totalClicks} Total Outbound Clicks
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[11px] text-slate-400">Live Network Webhooks</span>
          </div>
